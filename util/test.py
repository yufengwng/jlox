...
def init_interpreter():
    tests = {
        ...
        TEST_DIR + '/limit/too_many_locals.lox': 'skip',
        TEST_DIR + '/limit/too_many_upvalues.lox': 'skip',
...
class Test:
    ...
    def parse(self):
        ...
        with open(self.path, 'r') as file:
            for line in file:
                ...
                if match:
                    # The two interpreters are slightly different in terms of
                    # which cascaded errors may appear after an initial compile
                    # error because their panic mode recovery is a little
                    # different. To handle that, the tests can indicate if an
                    # error line should only appear for a certain interpreter.
                    language = match.group(2)
                    if not language or language == interpreter.language:
                        self.compile_errors.add("[{0}] {1}".format(match.group(3), match.group(4)))
    ...
    def validate_runtime_error(self, error_lines):
        ...
        # Skip any compile errors. This can happen if there is a compile error
        # in a module loaded by the module being tested.
        line = 0
        while SYNTAX_ERROR_RE.search(error_lines[line]):
            line += 1

        if error_lines[line] != self.runtime_error_message:
            self.fail('Expected runtime error "{0}" and got:', self.runtime_error_message)
            self.fail(error_lines[line])

        # Make sure the stack trace has the right line. Skip over any lines
        # that come from builtin libraries.
        match = False
        ...
        if not match:
            ...
            if stack_line != self.runtime_error_line:
                self.fail('Expected runtime error on line {0} but was on line {1}.', self.runtime_error_line, stack_line)
    ...
    def validate_exit_code(self, exit_code, error_lines):
        ...
        self.fail('Expected return code {0} and got {1}. Stderr:', self.exit_code, exit_code)
    ...
    def validate_output(self, out):
        ...
        for line in out_lines:
            ...
            if index >= len(self.output):
                ...
                self.fail('Expected output "{0}" on line {1} and got "{2}".', self.output[index][0], self.output[index][1], line)
            index += 1

        while index < len(self.output):
            self.fail('Missing expected output "{0}" on line {1}.', self.output[index][0], self.output[index][1])
            index += 1
...
def run_script(path):
    ...
    print_line('Passed: ' + green(passed) + ' Failed: ' + red(failed) +
            ' Skipped: ' + yellow(num_skipped) + gray(' (' + path + ')'))
